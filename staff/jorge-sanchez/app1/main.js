...
registerFormPasswordLabel.htmlFor = 'passowrd'
...
registerFormSubmitButton.type = 'sumbit'
...
loginFormPasswordLabel.htmlFor = 'passowrd'
...
loginFormSubmitButton.type = 'sumbit'
...
loginForm.onsubmit = function (event) {
    event.preventDefault()
    var usernameLoged = loginFormUsernameInput.value
    var passwordLoged = loginFormPasswordInput.value

    for (var i = 0; i < users.length; i++) {
        if (usernameLoged === users[i].username & passwordLoged === users[i].password) {
    ...
    // TODO validate credentials against users db (HINT find).
    // if credentials ok, then go to home. otherwise show alert with "wrong credentials"
}
